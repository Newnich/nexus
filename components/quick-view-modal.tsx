          ...
          <>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span
                      className={cn(
                        "text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase bg-nexus-500/10",
                        typeConfig.color,
                      )}
                    >
                      {typeConfig.label}
                  ...
                  <h2 className="text-lg font-bold gradient-text mt-1 break-words">
                    {item.title || "Untitled"}
                  </h2>
                </div>
              </div>
                ...
                {item.aiData?.sentiment && (
                  <>
                    ...
                    <span className="capitalize">
                      {item.aiData.sentiment === "positive"
                        ? "😊"
                        : item.aiData.sentiment === "negative"
                          ? "😟"
                          : "😐"}{" "}
                      {item.aiData.sentiment}
                    </span>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {connections.slice(0, 5).map((conn) => {
                      ...
                      const otherConfig =
                        ITEM_TYPE_CONFIG[other?.type as keyof typeof ITEM_TYPE_CONFIG] ||
            ...
            <div className="px-6 py-3 border-t border-border/50 flex items-center justify-between shrink-0">
              <span className="text-xs text-muted-foreground">
                {item.aiData?.keyPoints?.length || 0} key points · {item.aiData?.tags?.length || 0}{" "}
                tags
              </span>
              <Link
                href={`/items/${itemId}`}
