...
export default function NotificationSettingsPage() {
  ...
  const sendTest = async (channel: "slack" | "discord") => {
    ...
    setTestResults((prev) => ({
      ...prev,
      [channel + "-pending"]: { channel, sent: false, alertId: "pending" },
    }));
    ...
    <div className="space-y-8">
              ...
              <p className="text-sm font-mono">
                {slack?.urlPreview ? (
                  ...
                  <span className="text-muted-foreground/60 italic">
                    Set{" "}
                    <code className="text-xs bg-muted px-1 py-0.5 rounded">SLACK_WEBHOOK_URL</code>{" "}
                    in your environment
                  </span>
                )}
              </p>
      ...
      <div className="glass-card p-6 rounded-2xl">
        ...
        <div className="p-3 bg-muted/20 rounded-xl">
          ...
          <p className="text-sm font-mono">
            {email?.from || (
              <span className="text-muted-foreground/60 italic">Default: alerts@nexus.app</span>
            )}
          </p>
        </div>
        ...
        <p className="text-xs text-muted-foreground mt-3">
          ...
          <code className="text-xs bg-muted px-1 py-0.5 rounded">ALERT_EMAIL_FROM</code> in your
          environment.
        </p>
      </div>
      ...
      <details className="glass-card p-6 rounded-2xl group">
        ...
        <div className="mt-4 space-y-3 text-sm">
          <div className="p-3 bg-muted/20 rounded-xl">
            <code className="text-xs bg-muted px-1.5 py-0.5 rounded text-nexus-400">
              SLACK_WEBHOOK_URL
            </code>
                ...
                >
                  ...
                  >
                    ...
                    <span className="hidden md:inline">
                      {entry.sent ? "Sent" : entry.error ? "Failed" : "Error"}
                    </span>
                  </span>
                </div>
              ))}
      ...
      <div className="text-center text-xs text-muted-foreground/50">
        Notification channels are configured via environment variables · Changes require a server
        restart
      </div>
    </div>
  );
