          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${gradient} flex items-center justify-center text-2xl`}>
                  {typeConfig.icon}
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className={cn("text-[11px] font-semibold px-2.5 py-1 rounded-full uppercase bg-nexus-500/10", typeConfig.color)}>
                      {typeConfig.label}
                    </span>
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                <span className="flex items-center gap-1">📅 Saved {formatDateRelative(item.createdAt)}</span>
                <span className="text-border">·</span>
                <span className="flex items-center gap-1">✏️ Updated {formatDateRelative(item.updatedAt)}</span>
          ...
          {item.aiData?.tags && item.aiData.tags.length > 0 && (
            <div className="glass-card p-6 rounded-2xl">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <span className="text-xs text-muted-foreground ml-auto">{item.aiData.tags.length}</span>
              </div>
          ...
          <div className="glass-card p-6 rounded-2xl">
            ...
            <div className="space-y-3 text-sm">
              {[
                ...
                { label: "Visibility", value: item.visibility.charAt(0).toUpperCase() + item.visibility.slice(1) },
                ...(item.aiData?.language ? [{ label: "Language", value: item.aiData.language.toUpperCase() }] : []),
                ...(item.aiData?.processingVersion ? [{ label: "AI Version", value: `v${item.aiData.processingVersion}` }] : []),
              ].map(({ label, value }) => (
                <div key={label} className="flex justify-between items-center p-2 rounded-lg bg-muted/20">
            ...
            <div className="glass-card p-6 rounded-2xl">
              ...
              <div className="space-y-2">
                {connections.map((conn) => {
                  ...
                  const otherGradient = TYPE_GRADIENTS[otherType] || "from-nexus-500/20 to-indigo-500/20";
                  ...
                  return (
                    ...
                    >
                      <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${otherGradient} flex items-center justify-center text-base shrink-0`}>
                        {otherIcon}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          ...
                          <span className="text-[10px] text-muted-foreground font-mono">{strengthPct}%</span>
                        </div>
                      </div>
                    </Link>
                  );
